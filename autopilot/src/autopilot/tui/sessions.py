    ...
    def get_most_recent(self) -> Optional[Session]:
        ...
        with self._lock:
            ...
            sorted_sessions = sorted(
                sessions, key=lambda s: s.get("last_active", ""), reverse=True
            )
            return Session(**sorted_sessions[0])

    def find_by_id(self, execution_id: str) -> Optional[Session]:
        """Find a session by execution ID (full or prefix match).

        Raises ``ValueError`` if the prefix matches more than one session.
        """
        with self._lock:
            for s in self._data.get("sessions", []):
                if s["execution_id"] == execution_id:
                    return Session(**s)
            # Try prefix match — detect ambiguous matches
            matches = [
                s
                for s in self._data.get("sessions", [])
            ...
            if len(matches) == 1:
                ...
                raise ValueError(
                    f"Ambiguous session ID '{execution_id}' matches {len(matches)} sessions"
                )
            return None

    def cleanup(self, max_sessions: int = 50) -> None:
        """Remove oldest COMPLETED sessions if count exceeds *max_sessions*."""
        with self._lock:
            sessions = self._data.get("sessions", [])
            active = [s for s in sessions if s["status"] != "COMPLETED"]
            completed = [s for s in sessions if s["status"] == "COMPLETED"]
            if len(completed) > max_sessions:
                completed = sorted(
                    completed, key=lambda s: s.get("last_active", "")
                )[-max_sessions:]
            self._data["sessions"] = active + completed
            self._save()
