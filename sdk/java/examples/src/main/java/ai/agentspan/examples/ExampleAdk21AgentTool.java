 ...
 *
 * <p>Demonstrates: wrapping agents as callable tools via {@link AgentTool#from(Agent)}.
 * Unlike sub-agents (handoff), an agent_tool runs inline and returns its
 * output back to the parent like a function call.
 */
public class ExampleAdk21AgentTool {

    static class ResearcherTools {

        @Tool(name = "search_knowledge_base", value = "Search an internal knowledge base for information.")
    ...
    static class CalculatorTools {
        ...
        @Tool(name = "compute", value = "Evaluate a mathematical expression.")
        public Map<String, Object> compute(@P("expression") String expression) {
    ...
    public static void main(String[] args) {
        ...
        Agent calculator = GoogleADKAgent.builder()
            .name("calculator")
            .model(Settings.LLM_MODEL)
        ...
        Agent manager = GoogleADKAgent.builder()
            .name("manager")
            .model(Settings.LLM_MODEL)
            .instruction(
            ...
            .toolDefs(List.of(AgentTool.from(researcher), AgentTool.from(calculator)))
            .build();
