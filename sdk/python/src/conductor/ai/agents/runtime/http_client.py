...
class AgentClient:
    ...
    **Run is control-plane only.** :meth:`run` compiles + starts the agent and
    ...
    async def post_eval_run(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Short-lived client: these calls run via _run_sync(asyncio.run(...)),
        # which closes the event loop on return. A loop-bound shared client
        # would then raise "Event loop is closed" during teardown.
    ...
    async def push_dataset(
        self, name: str, cases: List[Dict[str, Any]], *, pushed_by: Optional[str] = None
    ) -> None:
