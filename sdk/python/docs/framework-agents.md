...
Agentspan `Runner` with an Agentspan `Agent`.
...
`Runner` methods (all classmethods, accept an OpenAI-Agents `Agent` or an Agentspan
`Agent`):
...
Run a Claude Agent SDK / Claude Code agent. The simplest path is an Agentspan `Agent`
