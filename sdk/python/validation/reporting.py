...
from .config import MODELS as _DEFAULT_MODELS
        ...
        for r in rows:
            ...
            for prefix in providers:
                if r.get(f"{prefix}_judge_score"):
                    label = prefix.title()
                    line = f"- **{label} judge**: {r[f'{prefix}_judge_score']}/5 — {r.get(f'{prefix}_judge_reason', '')}"
                    if r.get(f"{prefix}_baseline_score"):
                        line += f" | baseline: {r[f'{prefix}_baseline_score']}/5"
                    f.write(line + "\n")
