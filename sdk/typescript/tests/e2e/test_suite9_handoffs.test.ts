...
import { Agent, AgentRuntime, tool, OnTextMention } from "@agentspan-ai/sdk";
...
const doData = tool(async (args: { query: string }) => `data_result:${args.query}`, {
  name: "do_data",
...
describe("Suite 9: Agent Handoffs", { timeout: 1_800_000 }, () => {
  // 30 min for all multi-agent tests
  // ── Compilation tests ─────────────────────────────────────────────────
  ...
  it("sequential execution produces SUB_WORKFLOW tasks", async () => {
    ...
    for (const subWf of subWorkflows) {
      const subWfId =
        subWf.outputData?.subWorkflowId ?? (subWf as Record<string, unknown>).subWorkflowId;
      if (!subWfId) continue;
  ...
  it("parallel execution produces FORK task", async () => {
    ...
    const result = await runtime.run(parent, "Compute 3+4 AND reverse hello", { timeout: TIMEOUT });
  ...
  it("handoff execution delegates to sub-agent", async () => {
    ...
    const result = await runtime.run(parent, "I need to reverse the word hello", {
      timeout: TIMEOUT,
    });

    const diag = runDiagnostic(result as unknown as Record<string, unknown>);
    expect(result.executionId).toBeTruthy();
    expect(["COMPLETED", "FAILED", "TERMINATED"], `[Handoff] Unexpected status. ${diag}`).toContain(
      result.status,
    );
  ...
  it("router selects correct agent", async () => {
    ...
    const result = await runtime.run(parent, "Compute 7 times 8", { timeout: TIMEOUT });
    ...
    const tasks = await getWorkflowTasks(result.executionId);
    const subWorkflows = findTasksByType(tasks, "SUB_WORKFLOW");
    const mathSubs = subWorkflows.filter((t) =>
      String(t.referenceTaskName ?? "")
        .toLowerCase()
        .includes("math"),
    );
    expect(
  ...
  it("swarm with OnTextMention handoff", async () => {
    ...
    const result = await runtime.run(parent, "Please reverse the word hello", { timeout: TIMEOUT });

    const diag = runDiagnostic(result as unknown as Record<string, unknown>);
    expect(result.executionId).toBeTruthy();
    expect(["COMPLETED", "FAILED", "TERMINATED"], `[Swarm] Unexpected status. ${diag}`).toContain(
      result.status,
    );
    ...
    const tasks = await getWorkflowTasks(result.executionId);
    const subWorkflows = findTasksByType(tasks, "SUB_WORKFLOW");
    const textSubs = subWorkflows.filter((t) =>
      String(t.referenceTaskName ?? "")
        .toLowerCase()
        .includes("text"),
    );
  ...
  it("pipe operator creates sequential pipeline", async () => {
    ...
    // Run
    const result = await runtime.run(pipeline, "Compute 2+3 then reverse hello", {
      timeout: TIMEOUT,
    });
