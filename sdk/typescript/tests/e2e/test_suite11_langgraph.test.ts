...
describe("Suite 11: LangGraph Integration", { timeout: 300_000 }, () => {
  ...
  it.skipIf(!langGraphAvailable)("full extraction — react agent with 3 tools", () => {
    ...
    expect(String(rawConfig.model), "[FullExtract] wrong model").toContain("gpt-4o-mini");
    ...
    for (const t of toolsList) {
      expect(
        t.description,
        `[FullExtract] tool ${t.name ?? t._worker_ref} missing description`,
      ).toBeTruthy();
    }
  ...
  it.skipIf(!langGraphAvailable)("stategraph — 3 nodes and edges extracted", () => {
    ...
    function validate(state: any) {
      return { query: (state.query || "").trim() || "default" };
    }
    function process(state: any) {
      return { query: `processed:${state.query}` };
    }
    ...
    if (graphData) {
      ...
      expect(
        workerNames.some((n) => n.includes("validate")) || configStr.includes("validate"),
  ...
  it.skipIf(!langGraphAvailable)("conditional routing — conditional_edges extracted", () => {
    ...
    builder.addConditionalEdges("classify", routeQuery, {
      math: "handle_math",
    ...
    expect(rawConfig._graph, "[Conditional] _graph missing from rawConfig").toBeDefined();
    ...
    expect(condEdges.length, "[Conditional] conditional_edges should be non-empty").toBeGreaterThan(
      0,
    );
    ...
    expect(ce.source, `[Conditional] Expected source='classify', got '${ce.source}'`).toBe(
      "classify",
    );
  ...
  it.skipIf(!langGraphAvailable)("messages state detected — _input_is_messages flag", () => {
    ...
    const MessagesState = Annotation.Root({
      messages: Annotation<Record<string, unknown>[]>({
        reducer: (prev: Record<string, unknown>[], next: Record<string, unknown>[]) => [
          ...prev,
          ...next,
        ],
        default: () => [],
  ...
  it.skipIf(!langGraphAvailable)("compile stategraph via server", async () => {
    ...
    function validateQ(state: any) {
      return { query: (state.query || "").trim() || "default" };
    }
    function answerQ(state: any) {
      return { result: `answer_for:${state.query}` };
    }
  ...
  it.skipIf(!langGraphAvailable || !process.env.OPENAI_API_KEY)(
    "runtime execution — multiply tool returns 56",
    async () => {
      if (!serverAvailable) {
