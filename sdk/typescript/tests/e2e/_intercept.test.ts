...
it('trace what gets sent to server', async () => {
  ...
  (graph as any)._agentspan = { model: 'openai/gpt-4o-mini', tools: [calculateTool], framework: 'langgraph' };
  ...
  if (result?.executionId) {
    ...
    if (wfResp.ok) {
      ...
      for (const t of (wf.tasks || [])) {
        ...
        console.log(`  ${t.referenceTaskName} [${t.taskType}] ${status} ${reason ? '| ' + reason : ''}`);
      }
    }
  }
  if (!resp.ok) {
