...
import { Agent, AgentRuntime, tool, CallbackHandler } from "@agentspan-ai/sdk";
...
describe("Suite 13: Callbacks", { timeout: 300_000 }, () => {
  ...
  it("before_tool callback executes at runtime", async () => {
    ...
    expect(result.executionId, `[before_tool_callback] No executionId. ${diag}`).toBeTruthy();
    expect(
    ...
    const beforeToolTasks = allTasks.filter((t) =>
      ((t.referenceTaskName as string) ?? "").includes("before_tool"),
    );
  ...
  it("after_tool callback executes at runtime", async () => {
    ...
    expect(result.executionId, `[after_tool_callback] No executionId. ${diag}`).toBeTruthy();
    expect(
      ["COMPLETED", "TERMINATED"],
    ...
    const wf = await getWorkflow(result.executionId);
    const allTasks = (wf.tasks ?? []) as Array<Record<string, unknown>>;
    const afterToolTasks = allTasks.filter((t) =>
      ((t.referenceTaskName as string) ?? "").includes("after_tool"),
    );
  ...
  it("all callbacks do not block execution", async () => {
    ...
    expect(result.executionId, `[all_callbacks] No executionId. ${diag}`).toBeTruthy();
    expect(
      result.status,
