...
const addTool = tool(async (args: { a: number; b: number }) => ({ result: args.a + args.b }), {
  name: "add",
...
const multiplyTool = tool(async (args: { a: number; b: number }) => ({ result: args.a * args.b }), {
  name: "multiply",
...
const credentialedTool = tool(async (args: { query: string }) => ({ result: args.query }), {
...
const multiCredTool = tool(async (args: { data: string }) => ({ result: args.data }), {
...
const ksNoPii = guardrail((_content: string): GuardrailResult => ({ passed: true }), {
  name: "no_pii",
  position: "output",
  onFail: "retry",
});
...
function makeKitchenSinkAgent() {
  const localTool = tool(async (args: { x: string }) => args.x, {
    name: "local_tool",
  ...
  const credLocalTool = tool(async (args: { x: string }) => args.x, {
...
async function judgeCompiledWorkflow(
  comparison: string,
): Promise<{ pass: boolean; missing: string[]; explanation: string }> {
  let raw: string;
