 ...
 */
function getRegisteredWorkers(runtime: AgentRuntime): Array<{ taskName: string; handler: Function }> {
...
async function invokeWorker(
  ...
  if (!worker) {
    throw new Error(`Worker not found: ${taskName}. Registered: ${workers.map((w) => w.taskName).join(', ')}`);
  }
...
describe('OnToolResult.shouldHandoff', () => {
  ...
  it('checks resultContains when specified', () => {
    ...
    expect(cond.shouldHandoff({ result: '', toolName: 'check_order', toolResult: 'eligible for refund' })).toBe(true);
    expect(cond.shouldHandoff({ result: '', toolName: 'check_order', toolResult: 'not found' })).toBe(false);
  });
});
...
describe('_registerSwarmTransferWorkers', () => {
  ...
  it('returns error for unreachable targets with allowed_transitions', async () => {
    ...
    const errResult = (await invokeWorker(runtime, 'support_transfer_to_refund', {})) as Record<string, unknown>;
