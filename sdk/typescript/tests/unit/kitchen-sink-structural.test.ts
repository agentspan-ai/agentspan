...
import { TextMention, MaxMessage, TokenUsageCondition, OrCondition, AndCondition } from '../../src/termination.js';
...
describe('Stage 4: Review & Safety', () => {
  ...
  it('guardrail types include regex, llm, custom, external', () => {
    const types = reviewAgent.guardrails.map(
      (g) => (g as Record<string, unknown>).guardrailType,
    );
...
describe('Stage 8: Analytics & Reporting', () => {
  ...
  it('media tools have correct types', () => {
    const mediaToolDefs = analyticsAgent.tools.filter((t) => {
      ...
      return ['generate_image', 'generate_audio', 'generate_video', 'generate_pdf'].includes(def.toolType);
    });
...
describe('Serialization', () => {
  it('full pipeline serializes without error', () => {
    ...
    expect((payload.agentConfig as Record<string, unknown>).name).toBe('content_publishing_platform');
