...
- **Do NOT use mocks (Mockito) for internal services.** Tests that mock `SecretTagsService`, `UserRepository`, or any other internal service hide bugs at layer boundaries — the exact place bugs live. Use `@SpringBootTest` with the test profile's real SQLite DB instead.
