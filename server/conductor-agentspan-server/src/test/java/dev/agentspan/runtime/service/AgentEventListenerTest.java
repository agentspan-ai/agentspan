...
class AgentEventListenerTest {
    ...
    // ── onWorkflowStarted: implicit execution-token mint ─────────────

    private WorkflowModel makeRootWorkflow(
            String wfId, java.util.Map<String, Object> metadata, java.util.Map<String, Object> input) {
        WorkflowModel wf = new WorkflowModel();
        ...
        wf.setWorkflowDefinition(def);
        wf.setInput(input != null ? input : new java.util.HashMap<>());
        return wf;
    }

    @Test
    @SuppressWarnings("unchecked")
    void onWorkflowStarted_rootWithoutToken_mintsAndInjects() {
        ...
        AgentEventListener l = new AgentEventListener(streamRegistry, tokenService);
        ...
        WorkflowModel wf =
                makeRootWorkflow(
                        ...
                        Map.of(
                                ...
                                "agentspan_declared_credentials", java.util.List.of("OCG_PUBLIC_KEY", "github_pat")),
                        new java.util.HashMap<>(Map.of("prompt", "investigate")));

        l.onWorkflowStartedIfEnabled(wf);

        verify(tokenService)
                .mint(
                        eq("deployer-1"),
                        eq("wf-mint"),
                        eq(java.util.List.of("OCG_PUBLIC_KEY", "github_pat")),
                        anyLong());
        Map<String, Object> ctx = (Map<String, Object>) wf.getInput().get("__agentspan_ctx__");
        assertThat(ctx).isNotNull();
        assertThat(ctx.get("execution_token")).isEqualTo("minted-tok");
        // mirrored into variables for the revoke-on-completion path
        Map<String, Object> varCtx = (Map<String, Object>) wf.getVariables().get("__agentspan_ctx__");
        assertThat(varCtx.get("execution_token")).isEqualTo("minted-tok");
    }

    @Test
    void onWorkflowStarted_rootWithExistingToken_skipsMint() {
        ExecutionTokenService tokenService = mock(ExecutionTokenService.class);
        AgentEventListener l = new AgentEventListener(streamRegistry, tokenService);
        ...
        WorkflowModel wf =
                makeRootWorkflow(
                        "wf-sdk",
                        Map.of("agentspan_credential_user", "deployer-1"),
                        new java.util.HashMap<>(
                                Map.of("__agentspan_ctx__", new java.util.HashMap<>(Map.of("execution_token", "sdk-tok")))));
    ...
    @Test
    void onWorkflowStarted_rootWithoutMetadata_skipsMint() {
        ExecutionTokenService tokenService = mock(ExecutionTokenService.class);
        AgentEventListener l = new AgentEventListener(streamRegistry, tokenService);

        WorkflowModel wf = makeRootWorkflow("wf-nomd", null, new java.util.HashMap<>(Map.of("prompt", "x")));
    ...
    @Test
    void onWorkflowStarted_noStampedUserFallsBackToCreatedBy() {
        ExecutionTokenService tokenService = mock(ExecutionTokenService.class);
        ...
        WorkflowModel wf =
                makeRootWorkflow(
                        "wf-cb",
                        Map.of("agentspan_declared_credentials", java.util.List.of("OCG_PUBLIC_KEY")),
                        new java.util.HashMap<>(Map.of("prompt", "x")));
    ...
    @Test
    void onWorkflowStarted_subWorkflowDoesNotMint() {
        ExecutionTokenService tokenService = mock(ExecutionTokenService.class);
        AgentEventListener l = new AgentEventListener(streamRegistry, tokenService);
        ...
        WorkflowModel wf =
                makeRootWorkflow(
                        ...
                        Map.of("agentspan_credential_user", "deployer-1"),
                        new java.util.HashMap<>(Map.of("prompt", "x")));
        wf.setParentWorkflowId("parent-wf");
        wf.getWorkflowDefinition().setName("child_wf");

        l.onWorkflowStartedIfEnabled(wf);

        // Sub-workflows inherit the parent's token; they must not mint a new one.
        verify(tokenService, never()).mint(anyString(), anyString(), anyList(), anyLong());
    }
}
