...
-- Agentspan credential tables for PostgreSQL.
-- Loaded by CredentialDataSourceConfig when spring.datasource.url starts with jdbc:postgresql.
...
--   - All other column types are identical (TEXT, TIMESTAMP compatibility maintained)
...
CREATE TABLE IF NOT EXISTS credentials_store (
    user_id         TEXT    NOT NULL,
    name            TEXT    NOT NULL,
    encrypted_value BYTEA   NOT NULL,        -- AES-256-GCM ciphertext (PostgreSQL binary type)
    created_at      TEXT    NOT NULL,
