...
public class CredentialBindingService {
    ...
    public Optional<String> resolve(String userId, String logicalKey) {
        try {
            String storeName = jdbc.queryForObject(
                    "SELECT store_name FROM credentials_binding " + "WHERE user_id = :uid AND logical_key = :key",
    ...
    public void setBinding(String userId, String logicalKey, String storeName) {
        int updated = jdbc.update(
                "UPDATE credentials_binding SET store_name = :sn " + "WHERE user_id = :uid AND logical_key = :key",
                Map.of("sn", storeName, "uid", userId, "key", logicalKey));
        if (updated == 0) {
            jdbc.update(
                    "INSERT INTO credentials_binding (user_id, logical_key, store_name) " + "VALUES (:uid, :key, :sn)",
    ...
    public void deleteBinding(String userId, String logicalKey) {
        jdbc.update(
                "DELETE FROM credentials_binding WHERE user_id = :uid AND logical_key = :key",
                Map.of("uid", userId, "key", logicalKey));
    }
