...
public class JavaScriptBuilder {
    ...
    public static String enrichToolsScript(
        ...
        return iife("  var httpCfg = " + httpConfigJson + ";" + "  var mcpCfg = "
                ...
                // Convert inert #{NAME} credential markers to ${workflow.secrets.NAME} references
                // at dynamic-task emission. The '$' + '{...' split is load-bearing: the contiguous
                // secret-reference literal must never appear in this script's SOURCE (the INLINE
                // task's input), or conductor's substituteSecrets would resolve it to plaintext at
                // the INLINE's hand-off and the plaintext would persist via the script's output.
    ...
    /**
     * Format OCG search hits into a system-message text block for injection
     * into the agent's prompt. Reads {@code $.memories} (the
     * {@code response.body.memories} array from the search HTTP task) and folds
     * the human good/bad signal into each line — mirroring the Python
     * {@code _with_signal}. Returns {@code ""} when there are no hits, so the
     * injected system message is harmless when memory is empty.
     */
    public static String formatMemorySearchScript() {
        return iife("  var mems = $.memories;"
    ...
    /**
     * Parse the distiller LLM's JSON output and build the durable memory
     * ``value`` string. The {@code LLM_CHAT_COMPLETE} task exposes its result as
     * a JSON <em>string</em> at {@code output.result} (``jsonOutput`` only nudges
     * the model to emit JSON; the server does not parse it), so this reads the
     * raw string {@code $.distilled}, strips any prose/code-fence around the
     * object, and {@code JSON.parse}s it. Mirrors the Python post-run save which
     * appends a ``Facts:`` block. Returns {@code {value, description, summary,
     * facts, tags}} so the save HTTP body and the feedback_sink task can read
     * the parsed fields from this one task (the distiller's
     * {@code output.result.summary} would never resolve — it is a string).
     * Resilient: malformed JSON falls back to using the raw text as the summary.
     */
    public static String buildMemoryValueScript() {
        return iife("  var raw = $.distilled;"
                + "  var obj = {};"
