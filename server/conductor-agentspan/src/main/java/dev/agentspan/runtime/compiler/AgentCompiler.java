...
public class AgentCompiler {
    ...
    /**
     * Distillation prompt for the long-term (OCG) memory summarizer LLM step.
     * Kept in sync with the Python {@code MEMORY_SUMMARIZER_INSTRUCTIONS} in
     * {@code ocg_memory.py}.
     */
     ...
     * <ol>
     *   <li>{@code *_ltm_search} — HTTP POST {@code /api/v1/memories/search}
     *       (feedback-blended ranking).</li>
     *   <li>{@code *_ltm_format} — INLINE (GraalJS) that formats the hits into a
     *       text block (folding the good/bad signal) per
     *       {@link JavaScriptBuilder#formatMemorySearchScript()}.</li>
     *   <li>{@code *_ltm_set_context} — SET_VARIABLE stashing the formatted block
     *       into {@code _ltm_context} for the LLM system message.</li>
     * </ol>
    ...
    List<WorkflowTask> buildLtmRetrievalTasks(AgentConfig config) {
        ...
        WorkflowTask searchTask = buildMemoryHttpTask(
                searchRef, ltm.getOcgUrl() + "/api/v1/memories/search", "POST", ltm.getCredential(), searchBody);
     ...
     * <ol>
     *   <li>{@code *_ltm_distill} — LLM_CHAT_COMPLETE that summarizes the ticket +
     *       final report into a {@link dev.agentspan.runtime.model.MemorySummary}-shaped
     *       JSON ({@code summary}, {@code facts}, {@code tags}).</li>
     *   <li>{@code *_ltm_build_value} — INLINE building the durable memory ``value``
     *       string from summary + facts.</li>
     *   <li>{@code *_ltm_save} — HTTP POST {@code /api/v1/memories}.</li>
     *   <li>{@code *_ltm_feedback_links} — HTTP POST
     *       {@code /api/v1/memories/{key}/feedback-links}.</li>
     *   <li>{@code <feedbackSink.taskName>} — SIMPLE worker handing the links to the
     *       user's Python feedback_sink (only when {@code feedbackSink} is set).</li>
     * </ol>
     * All tasks are {@code optional=true} (best-effort).
     *
    ...
    List<WorkflowTask> buildLtmSaveTasks(AgentConfig config, String finalOutputRef) {
        ...
        // 1. Distill LLM task — summarize the run into durable facts.
        String distillRef = base + "_ltm_distill";
        WorkflowTask distillTask = buildMemoryDistillTask(distillRef, ltm.getSummaryModel(), finalOutputRef);
        ...
        WorkflowTask saveTask = buildMemoryHttpTask(
                saveRef, ltm.getOcgUrl() + "/api/v1/memories", "POST", ltm.getCredential(), saveBody);
        tasks.add(saveTask);
        ...
        WorkflowTask linksTask =
                buildMemoryHttpTask(linksRef, linksUri.toString(), "POST", ltm.getCredential(), null);
        tasks.add(linksTask);
    ...
    /**
     * Build an HTTP task targeting the OCG BFF. The credential is written as a
     * {@code ${NAME}} placeholder and rewritten by {@link ToolCompiler#escapeCredentialHeaders}
     * into whatever the host resolves: embedded (orkes-conductor) →
     * {@code ${workflow.secrets.NAME}} (host secret store — same path the OCG query
     * tools use), standalone → {@code #{NAME}} (resolved by {@code CredentialAwareHttpTask}
     * against the per-user execution token). Hardcoding {@code #{NAME}} here was wrong
     * for the embedded host and produced 401s. Marked {@code optional=true} so memory
     * failures never fail the agent. A {@code null} body sends no JSON body.
     */
    WorkflowTask buildMemoryHttpTask(
            String refName, String uri, String method, String credential, Map<String, Object> body) {
        ...
        // Host-mode-aware rewrite: ${NAME} -> ${workflow.secrets.NAME} (embedded) or #{NAME} (standalone).
        ...
        // Forward execution token so CredentialAwareHttpTask resolves #{NAME} headers.
     ...
     * MemorySummary-shaped JSON ({@code summary}, {@code facts}, {@code tags}).
     * Forces JSON output. Marked {@code optional=true}.
     */
    WorkflowTask buildMemoryDistillTask(String distillRef, String summaryModel, String finalOutputRef) {
        ...
        String systemMessage = MEMORY_SUMMARIZER_INSTRUCTIONS
        ...
        // Forward execution token so per-user credential resolution works.
        inputs.put("__agentspan_ctx__", "${workflow.input.__agentspan_ctx__}");
