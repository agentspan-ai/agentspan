...
public class AgentService {
    ...
    /**
     * Search executions with an optional {@code classifier} filter (folded in as
     * {@code classifier IN (...)}) and an optional top-level-only restriction. Top-level executions
     * are roots (no parent); roots store {@code parent_workflow_id = ""}, so the restriction is the
     * filter {@code parentWorkflowId = ""}. Both are ANDed onto the caller's {@code query}.
     */
    public SearchResult<WorkflowSummary> searchExecutionsRaw(
            int start, int size, String sort, String freeText, String query, String classifier, boolean topLevelOnly) {
        String effectiveQuery = withClassifierFilter(query, classifier);
        if (topLevelOnly) {
            String topLevelFilter = "parentWorkflowId = \"\"";
            effectiveQuery = (effectiveQuery == null || effectiveQuery.isBlank())
                    ? topLevelFilter
