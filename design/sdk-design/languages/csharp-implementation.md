...
Adapters convert a framework-shaped declaration into a Conductor Agents `Agent`; the server normalizers (`OpenAINormalizer`, `GoogleADKNormalizer`) consume the resulting wire shape. See [`../../framework-integration.md`](../../framework-integration.md).
...
- **Dependency on `conductor-csharp`.** Worker polling reuses the official client's `Configuration`, `TaskResourceApi`, `TaskResult`, and (for Orkes) `OrkesAuthenticationSettings` rather than reimplementing the task protocol. The agent control plane (`/agent/*`) is bespoke (`AgentClient`) because those endpoints are Conductor Agents-specific.
